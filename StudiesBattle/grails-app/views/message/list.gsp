			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="contenu" title="${message(code: 'message.contenu.label', default: 'Contenu')}" />
					
						<th><g:message code="message.auteur.label" default="Auteur" /></th>
					
						<th><g:message code="message.mur.label" default="Mur" /></th>
				...
				<g:each in="${messageInstanceList}" status="i" var="messageInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${messageInstance.id}">${fieldValue(bean: messageInstance, field: "contenu")}</g:link></td>
					
						<td>${fieldValue(bean: messageInstance, field: "auteur")}</td>
					
						<td>${fieldValue(bean: messageInstance, field: "mur")}</td>
